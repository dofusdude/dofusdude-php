...
```php
getAllItemsResourcesList($language, $game, $sort_level, $filter_type_name, $filter_min_level, $filter_max_level, $accept_encoding, $filter_type_enum): \OpenAPI\Client\Model\ItemsListPaged
```
...
try {
    $result = $apiInstance->getAllItemsResourcesList($language, $game, $sort_level, $filter_type_name, $filter_min_level, $filter_max_level, $accept_encoding, $filter_type_enum);
    print_r($result);
} catch (Exception $e) {
