...
```php
getMountsSearch($language, $game, $query, $filter_family_name): \OpenAPI\Client\Model\MountListEntry[]
```
...
try {
    $result = $apiInstance->getMountsSearch($language, $game, $query, $filter_family_name);
    print_r($result);
} catch (Exception $e) {
